...
def inject_null_layer(network, inject_after=0):
    if 0 <= inject_after and inject_after < len(network):
        inject_size = network[inject_after][0].shape[0]
        return (
            network[:inject_after+1]+
            [[np.identity(inject_size), 0]]+
...
def inject_null_neuron(network, inject_in=0):
    ...
    if 0 <= inject_in and inject_in < network_size:
        inject_size = network[inject_in][0].shape[1]
        injected_neuron = np.zeros((1, inject_size))
        if inject_in < network_size-1:
            mutation_size = network[inject_in+1][0].shape[0]
            ...
            return (
                ...
                [[
                    np.concatenate(
                        (
                            network[inject_in+1][0],
                            mutated_neurons,
                        ),
                        axis=1
                    ),
                    network[inject_in+1][1]
                ]] +
                network[inject_in+2:]
            )
        return (
